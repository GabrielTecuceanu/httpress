...
impl Metrics {
    ...
    /// # Arguments
    ///
    /// * `buckets` - A slice of Duration values representing bucket upper bounds.
    ///   Each bucket includes latencies greater than the previous bucket's bound
    ///   and less than or equal to the current bucket's bound.
    ///   A final bucket is always added for values exceeding the last bound.
    ///
    /// # Returns
    ///
    ...
    pub fn print_histogram(&self) {
        ...
        let histogram = self.histogram(&buckets);
